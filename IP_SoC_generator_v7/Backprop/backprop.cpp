/*******************************************************************************
 David Berrazueta M.
 Associated Filenames: backprop.cpp, backprop.h, backprop_test.cpp
 Purpose: Vivado HLS accelerator 4
 Device: Xilinx Zynq-7000 SoC ZC702 Evaluation Kit
 Clock Period: 10ns
 Clock Uncertainty: default (12.5%)
 Revision History: December 4, 2018
...
void soft_max(TYPE net_outputs[possible_outputs],
	...
	for (i = 0; i < possible_outputs; i++) {
		sum += exp(double(-activations[i]));
	}
	for (i = 0; i < possible_outputs; i++) {
		net_outputs[i] = exp(double(-activations[i])) / sum;
	}
}
...
void RELU(TYPE activations[nodes_per_layer],
	...
	for (i = 0; i < size; i++) {
		...
		activations[i] = 1.0 / (1.0 + exp(double(-activations[i])));
	}
}
