...
int main() {
	...
	int res_sw[MAT_A_ROWS][MAT_B_COLS],res_hw[MAT_A_ROWS][MAT_B_COLS];
	int status;
	int buffer=0;
	...
	for (i = 0; i < 2; i++) {
		for(j = 0; j <2; j++){
			data=a[i][j];
			XMatrixmul_Write_a_Words(&Hls_matrixmul, (i*2)+j, &data, 1);
		}
	}
	...
	for (i = 0; i < 2; i++) {
		for(j = 0; j <2; j++){
			data=b[i][j];
			XMatrixmul_Write_b_Words(&Hls_matrixmul, (i*2)+j, &data, 1);
		}
	}
	...
	for (i = 0; i < 2; i++) {
		for(j = 0; j <2; j++){
			XMatrixmul_Read_res_Words(&Hls_matrixmul, (i*2)+j, &buffer, 1);
			res_hw[i][j]=buffer;
			printf("res :%d\n",buffer);
		}
	}
	...
	printf("Result from HW: %d; Result from SW: %d\n\r", res_hw[1][1],
			res_sw[1][1]);
	if ((res_hw[0][0] == res_sw[0][0]) & (res_hw[0][1] == res_sw[0][1])
			& (res_hw[1][0] == res_sw[1][0]) & (res_hw[1][1] == res_sw[1][1])) {
		print("*** Results match ***\n\r");
		status = 0;
	} else {
		print("!!! MISMATCH !!!\n\r");
